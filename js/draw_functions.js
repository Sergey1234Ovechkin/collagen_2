...
//добавляет RGBA -эффект с помощью функции - script к границе
//area - массив с точками выделения
//iteration, количество итераций lineW, тилщина линии*2 border_inner_outer_box,напрвление наружу внутрь step_iteration шаг итерации в пикселях*2
function addEffectEvalToBorderPixels(ctx, area,  script, iteration, lineW, border_inner_outer_box, step_iteration){
	
	//var lineW = 20;
	var coeff_inner_outer = lineW;
	if(border_inner_outer_box)coeff_inner_outer = 0;
	var temporaryObj = {
		iteration: 0,
	};
	var imgBox = getBox(area); 	
	var cutWidth = imgBox[1][0] - imgBox[0][0]; var cutHeight = imgBox[1][1] - imgBox[0][1];
	var cutArea = getCutSize(area, imgBox[0][0]-coeff_inner_outer, imgBox[0][1]-coeff_inner_outer); 
	//var cutArea2 = scaleArea(cutArea, coeff_x, coeff_y);	
	var cutPathArea =  getPathArea(cutArea);
	//var cutPathArea2 =  getPathArea(cutArea2); 
	ctx.lineWidth = lineW;
	//ctx.fill(cutPathArea);
	//ctx.stroke(cutPathArea);
	
	ctx.putImageData(saveImg, 0, 0);
	var H , W;	
	H = cutHeight+coeff_inner_outer*2; W = cutWidth+coeff_inner_outer*2;		
	var imgMap = ctx.getImageData(imgBox[0][0]-coeff_inner_outer, imgBox[0][1]-coeff_inner_outer, cutWidth+coeff_inner_outer*2 , cutHeight+coeff_inner_outer*2);  	
	    for(var it = 0; it< iteration; it ++ ){
			temporaryObj.iteration+=1;
			ctx.lineWidth -=step_iteration;
			if(!border_inner_outer_box){
			  for(var tmpX = 0; tmpX <  W; tmpX++) {
				for(var tmpY = 0;  tmpY < H; tmpY++) {
					
					var point = (tmpY*W+tmpX)*4; 								
					if(!ctx.isPointInPath(cutPathArea, tmpX, tmpY) &&  ctx.isPointInStroke(cutPathArea, tmpX, tmpY)){							
			...
			ctx.putImageData(imgMap, imgBox[0][0]-coeff_inner_outer, imgBox[0][1]-coeff_inner_outer);
...
//создание кода
function createCode(script, context, sprites, sprites_group){
	var this_ = context;
	eval(script);
}
...
function getImgToSprite(imgMapArr, sprite, isRender, mirror){
   Promise.all([
	...
	if(mirror === undefined || mirror != false ){sprite.frame_collection.push(sprite.frame);
	   }else{
		   sprite.frame_collection[sprite.frame_index] = sprite.frame;		   
	   }
...
function rotationArea(area, fi){
	...
	for(var i=0; i< cutArea.length; i++){
		...
		//if(cutArea[i][2] !== undefined)newArr[i][2] = cutArea[i][2];		
		    if(cutArea[i][2] !== undefined){
			   //var pos_2 = pos.slice(2);
			   newArr[i] = newArr[i].concat( cutArea[i].slice(2) );
			   //arr[2]=pos[2];	
		    }	
...
function drawSpriteGrid(x, y){ //рисует сетку спрайтов

    var w = canvas.width;
	var h = canvas.height;
	for(var i = x; i<w; i+=x){
		drawLine([i, 0 ], [i, h], grid_color, grid_weight);
	}
    for(var i = y; i<h; i+=y){
		drawLine([0, i ], [w, i], grid_color, grid_weight);
	}
