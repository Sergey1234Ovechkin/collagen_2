...
var gameUrl = window.location.origin + "/collagen_2/games/forest_3/sprites.json";
console.log(gameUrl);
...
// modules.mousedown =  moveMouse;
 function  moveMouse(point){
	 ...
	 smothStep =  Math.abs(distance[0]/distance[1]);
      ...
      //console.log(direction_, distance);
	  clearInterval(timerId);
     timerId = setInterval(movePersonage_2, interval);	 	
 }
/// движение персонажа кликами мыши
 function movePersonage_2(){
	  var dy = moveStep*smothStepY*( distance[1]/Math.abs(distance[1]) );
	  var dx =  moveStep*smothStepX*( distance[0]/Math.abs(distance[0]) );
	   ctxTranslate[0] += dx*-1; ctxTranslate[1] += dy*-1;
	  //console.log(dx, dy);
     if(direction_ == "right"){		
		 move(7, 11, 8, dx, dy);
	     if(ctxTranslate[0] <= ctxTranslate_2[0]){
			 clearInterval(timerId);
		 } 
	 }
     if(direction_ == "left"){
     ...
     if(direction_ == 'bottom'){
		 move(-1, 3, 0, dx, dy);
	     if(ctxTranslate[1] <= ctxTranslate_2[1]){
			 clearInterval(timerId);
		 } 
	 } 
     if(direction_ == 'top'){
...
///добавляем контейнер с формой в для отправки сообщений в объект StateMap  /js/games/inteface_games.js
 StateMap.user_message = { 
		...
		methods: {
			...
			move_type_btn: function(){ ///выбор способа управления персонажем
 ...
 StateMap.chose_personage = { 	
		...
		methods: {
			personage_1: function(){
				...
				modules.personage =  addTileCommon(personageId, this.$props().sprites["personage_1"], [250, 300]);
				pesonageType = "personage_1";
				createSocket();
            },
			personage_2: function(){
				this.parent.htmlLink.style.display = "none";
				modules.personage =  addTileCommon(personageId, this.$props().sprites["personage_2"], [250, 300]);
